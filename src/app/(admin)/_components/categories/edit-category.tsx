...
export const EditCategoryComponent = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (values: zod.infer<typeof EditCategorySchema>) => updateCategory(id as string, values),
    onSuccess: (d) => {
      if (d.data?.code === 200) {
  ...
  return (
    <div>
      ...
      {category && (
        ...
        <Form {...form}>
          <PageTitle icon={Edit} label={<span className='flex items-center gap-2'>Edit Category <Link className='text-sm text-gray-400 font-normal' href={`/admin/categories/${category?._id}/view`}>/{category?.title} ({category.slug})</Link></span>} />

          <form onSubmit={form.handleSubmit(updateCategoryHandler)} className='flex flex-col gap-y-3 mt-5 w-[50%]'>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input defaultValue={category?.title} {...field} type='text' placeholder='Name' />
                  </FormControl>
                  <FormDescription />
                  <FormMessage />
                </FormItem>
