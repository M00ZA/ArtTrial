  ...
  const ProfileImage = (
    <div className='flex gap-4 items-center'>
      <Avatar className='size-24'>
        <AvatarImage src={artist?.profileImg?.secure_url} alt="@shadcn" />
        <AvatarFallback>{artist?.name[0]}</AvatarFallback>
      </Avatar>
      <div>
        <span className='text-xl flex gap-2 items-center'>
          <span>{artist.name}</span>
          <span className='text-gray-400 font-medium text-xs'>{artist.accountActive ? 'Activated' : 'Not Activated'}</span>
        </span>
        <span className='block text-xs text-gray-400 font-medium'>{artist.email}</span>
      </div>
    </div>
  )
  ...
  return (
    <div>
      <PageTitle label={ProfileImage}>
        <Link href={`/admin/artists/${artist._id}/edit`}><Button className='flex gap-2' variant='outline'>Edit</Button></Link>
        <Link href={`/admin/artists/${artist._id}/delete`}><Button className='flex gap-2' variant='destructive'>Delete</Button></Link>
      </PageTitle>
      ...
      <ul className="divide-y">
        <li className='flex justify-between py-1'>
          ...
          <span>{artist?._id}</span>
        </li>
        <li className='flex justify-between py-1'>
          <span className='flex gap-2 items-center'><Mail className='size-4' /> Email</span>
          <span>{artist?.email}</span>
        </li>
        <li className='flex justify-between py-1'>
          <span className='flex gap-2 items-center'><Smartphone className='size-4' /> Phone</span>
          <span>{artist?.phone}</span>
        </li>
        <li className='flex justify-between py-1'>
          <span className='flex gap-2 items-center'><Check className='size-4' /> Is Active?</span>
        ...
        <li className='flex justify-between py-1'>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Addresses of <b>{artist.name}</b></AlertDialogTitle>
                <AlertDialogDescription>{artist.addresses?.length} addresses found</AlertDialogDescription>
