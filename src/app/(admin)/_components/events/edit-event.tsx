...
export const EditEventComponent = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (values: zod.infer<typeof EditEventSchema>) =>
      updateEvent(id as string, values),
    onSuccess: (d) => {
      if (d.data?.code === 200) {
          ...
          >
            ...
            {!allProducts.isLoading && products?.length >= 3 ? (
              ...
              <Alert variant="destructive">
                <X className="h-4 w-4" />
                <AlertTitle>
                  Owner Doesn't have 3 products to update this event
                </AlertTitle>
                <AlertDescription>
                  This owner must have at least 3 products to update this event,
                  please inform him!
                </AlertDescription>
              </Alert>
            )}
