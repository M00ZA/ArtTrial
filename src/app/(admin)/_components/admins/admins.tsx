...
export const AdminsComponent = () => {
  ...
  console.log("AAAAADDDDMin", admin);

  const admins = adminsQuery.data?.data?.data?.admins?.filter(
    (a: Admin) => a.id != admin.id
  );
    ...
    <div>
      <PageTitle label={translate("admins")} icon={Lock}>
        <Button className="flex gap-2">
          <Link href="/admin/admins/add" className="flex gap-2">
            <Plus className="size-5" /> {translate("new")}
          </Link>
        </Button>
      </PageTitle>

      <DataTable rows={admins} columns={AdminsColumns} />
    </div>
  );
};
