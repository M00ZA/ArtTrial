...
export const DeleteUserComponent = () => {
  ...
  return (
    <div>
      <div className="w-[50%]">
        <PageTitle icon={Trash} label={<span className='flex items-center gap-2'>Delete User <Link className='text-sm text-gray-400 font-normal' href={`/admin/users/${user?._id}`}>/{user?.name} ({user.email})</Link></span>} />
        <DeleteAction 
          icon={Trash}
